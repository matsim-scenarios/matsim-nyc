...
import org.matsim.codeexamples.scoring.example16customscoring.NewScoring;

import javax.inject.Inject;

public class RunCustomScoringExampleTaxi {
	
	
	public static final class TaxiScoringFunctionFactory implements ScoringFunctionFactory {
		private final  Scenario scenario;
		//private final ObjectAttributes personAttributes;

		public TaxiScoringFunctionFactory(Scenario scenario) {
...
//			}
			...
			sumScoringFunction.addScoringFunction(new NewScoring()); 
...
//		}
	...
	static class TaxiEngine implements PersonLeavesVehicleEventHandler, LinkEnterEventHandler {
		...
		public void handleEvent(PersonLeavesVehicleEvent event) {
			...
			if (taxiAt(event.getTime(), event.getVehicleId())) {
		...
		private boolean taxiAt(double time, Id<Vehicle> VehicleId) {
			if (time > (0.5 * 60.0 * 60.0) && VehicleId.toString().contains("1")) {
				return true;
			} else {
				return false;
			}
